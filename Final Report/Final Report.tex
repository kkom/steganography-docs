...
\label{sec:arbitrary_source_stego}
...
Using the interval algorithm, based on $\mathbf I$ we can deterministically generate an arbitrarily long sequence $\mathbf S$ from the stegotext source $\mathcal S$. It remains to show that a sufficiently long $\mathbf S$ allows us to find a correct sequence $\mathbf P'$. Remember that $\mathbf{P'}$ is defined as a sequence on $\mathfrak P$ whose prefix is $\mathbf P$.

To recover $\mathbf P$ from $\mathbf S$, we can run the interval algorithm in reverse. The only requirement to correctly decode all symbols of $\mathbf P$ is that $\mathbf s \subseteq \mathbf p$. This is the reason we use the process $\mathcal R$ during generation of $\mathbf S$ -- we keeping refining the input interval $\mathbf i$ until it gets small enough that $\mathbf s \subseteq \mathbf p$. $\mathbf s$ is guaranteed to decrease every time a new symbol is appended to $\mathbf S$ since the conditional probabilities are always less than 1.\footnote{Each symbol has a non-zero probability and the alphabet size of obviously more than 1.} \cref{fig:stegosystem_encoding} illustrates this with a concrete example.
...
\begin{figure}[h]
	...
	\caption{\label{fig:stegosystem_encoding}Generating $\mathbf S$ (right) from $\mathbf P$ (left). Both $\mathcal P$ and $\mathcal S$ are arbitrary stochastic processes defined over the alphabets $\mathfrak P = \{a,b,c\}$ and $\mathfrak S = \{\alpha,\beta,\gamma,\delta\}$. $\mathcal R$ emits \iid uniform bits. Statistics of the corresponding processes are used to provide mappings between sequences and intervals. We are given $\mathbf P = [b,c]$ and we wish to generate $\mathbf S$ from it. $\mathbf P$ corresponds to the light grey region $\mathbf p = [0.34,0.69)$, which only allows to generate $\mathbf S$ up to $[\gamma]$. Since $[0.30,0.87) \nsubseteq [0.34,0.69)$, $[\gamma]$ is not enough to identify a sequence with prefix $\mathbf P$. However, if we use $\mathcal R$ to extend $\mathbf P$ to $\mathbf I = [b,c,0,1]$, we obtain the refined input interval $\mathbf i = [0.45, 0.53)$ shown in dark grey. $\mathbf i$ allows us to generate $\mathbf S = [\gamma,\beta]$ with the required property that $\mathbf s \subseteq \mathbf p$.}
...
\begin{figure}[h]
	...
	\caption{\label{fig:stegosystem_decoding}Generating $\mathbf{P'}$ (right) from $\mathbf S$ (left). We use the statistics of $\mathcal S$ to find the interval $\mathbf s = [0.42,0.59)$. We then use the interval algorithm to generate $\mathbf{P'}$. Since we don't know how long the original sequence $\mathbf P$ was, we keep generating terms according to the statistics of $\mathcal P$ for as long as possible. In this case we decode $\mathbf{P'} = [b,c,b]$. $\mathbf{P'}$ has the prefix $\mathbf P = [b,c]$, but also contains an extra symbol $b$.}

\end{figure}

\FloatBarrier
\subsection{Innocuous transmission of text with deniable encryption}

The system described in \cref{sec:arbitrary_source_stego} can be used as a part of a bigger system, which \emph{securely} and \emph{innocuously} transmits arbitrary English text $\mathbf P$ using different English text $\mathbf K$ as a \emph{key}.

I will describe a modular stegosystem in which the \emph{plaintext} and the \emph{key} are first mapped to intervals $\mathbf p$ and $\mathbf k$ in the usual way and then to binary sequences $\mathbf{P_2}$ and $\mathbf{K_2}$. $\mathbf{P_2}$ is a \emph{binary subinterval sequence} of $\mathbf p$ and $\mathbf{K_2}$ is a \emph{binary superinterval sequence} of $\mathbf k$ -- see \cref{fig:plaintext_to_binary,fig:key_to_binary} for definitions and examples.

The binary representation of plaintext $\mathbf{P_2}$ is then encrypted to binary \emph{ciphertext} $\mathbf{C_2}$ using a stream cipher with key $\mathbf{K_2}$. If $\mathbf{K_2}$ is not shorter than $\mathbf{P_2}$, we achieve perfect secrecy \cite{shannon1949communication}. The cipher needs to ensure that $\mathbf{C_2}$ is \iid uniform. $\mathbf{C_2}$ is then used as an input to the system from \cref{sec:arbitrary_source_stego}, producing pseudo-random English \emph{stegotext} $\mathbf S$. Provided that the language model is correct, the stegotext is statistically indistinguishable from any random English text.

Running the system from \cref{sec:arbitrary_source_stego} in reverse, $\mathbf S$ allows us to recover a binary sequence $\mathbf{C'_2}$ whose prefix is the ciphertext $\mathbf{C_2}$. Knowing the key and its binary representation $\mathbf{K_2}$, we can recover the original binary plaintext sequence potentially followed by more bits, \ie $\mathbf{P'_2}$. Using the language model, we can now recover $\mathbf{P'}$ -- the original plaintext potentially followed by extra tokens.

An important feature of a stegosystem constructed in this way is \emph{deniable encryption}. Assume that an enemy with full knowledge of the system is told that a particular fragment of text $\mathbf S$ is indeed stegotext. Using the knowledge of the system (\ie the language model), they can reconstruct $\mathbf {C'_2}$ -- the binary ciphertext sequence with trailing bits. However, the recovery of plaintext requires knowledge of the key. If a \emph{false key} $\mathbf {\overline K}$ is supplied in text form, it will still be correctly converted to some binary sequence $\mathbf {\overline K_2}$. It will then allow to decipher without failure ciphertext $\mathbf{C'_2}$ into an \emph{incorrect} binary sequence $\mathbf {\overline {P'}_2}$. $\mathbf {\overline {P'}_2}$ will then be mapped to some incorrect plaintext $\mathbf {\overline {P'}}$, generated according to the stochastic process $\mathcal P$. So it will be a fragment of text randomly drawn from all English sentences. As a result, a user asked to supply the key may give $\mathbf {\overline K}$ instead and claim that $\mathbf {\overline {P'}}$ contains the plaintext message as a prefix. If the enemy does not know $\mathbf P$ but expects it to be just a random sentence, \ie one generated according to the same statistics as $\mathcal P$, $\mathbf {\overline {P'}}$ is just as likely to contain $\mathbf P$ as a prefix as is the real $\mathbf{P'}$.

Note that this is a special case where both \emph{plaintext} and \emph{stegotext} are generated according to the same model. So transmitting stegotext has the same innocuousness as transmitting plaintext. However, the system still implements secrecy without \emph{compromising} innocuousness. Conversely, standard text encryption gives a sequence of \iid uniform bits, not just different plausible text. Also, one can easily imagine modifying the system to use a completely different stochastic process for plaintext, \eg French text. Plaintext source could be also much more elaborate, for example it could be a custom format for storing some intelligence information. If $\mathcal P$ is designed to emit plausible outputs, we have deniable encryption.

\subsubsection{Example}

Assume that the language contains three tokens $\{ \alpha, \beta, \gamma \}$ and both plaintext and stegotext are generated according to the stochastic process $\mathcal E$. The plaintext we wish to securely and innocuously transmit is $\mathbf P = [\beta,\gamma]$. For this purpose we use the key $\mathbf K = [\beta,\alpha]$.
...
\begin{figure}[h]
	...
	\caption{\label{fig:plaintext_to_binary}Finding a \emph{binary subinterval sequence} of $\mathbf p$ (left), \ie the shortest sequence $\mathbf{P_2}$ (right) such that $\mathbf {p_2} \subseteq \mathbf p$. Mapping between binary sequences and intervals is achieved in the usual way -- by successive refinements of the $[0,1)$ interval by its upper or lower half. Mapping between $\mathbf P$ and $\mathbf p$ is achieved using arithmetic coding and the process $\mathcal E$. Note that there may be more than one valid binary subinterval sequence. In this example $\mathbf p = [0.49,0.69)$ and $\mathbf{P_2} = [1,0,0]$.}
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[scale=0.05]
		% Show that the binary interval is a subinterval of plaintext interval
		\fill [opacity=0.1] (-30,0.28*100) rectangle (30,0.39*100);
		...
		% Output side
		\intervalBothLabels{90}{0}{100}{0}{1}{solid}{right}
		\node[below] at (90,-5) {\footnotesize $[~]$};

		\intervalBothLabels{60}{0}{100}{0.50}{1.00}{densely dotted}{right}
		\intervalBottomLabel{60}{0}{100}{0.00}{0.50}{solid}{right}
		\node[below] at (60,-5) {\footnotesize $[0]$};

		\intervalBothLabels{30}{0}{100}{0.25}{0.50}{solid}{right}
		\intervalBottomLabel{30}{0}{100}{0.00}{0.25}{densely dotted}{right}
		\node[below] at (30,-5) {\footnotesize $[0,1]$};

	\end{tikzpicture}

	\caption{\label{fig:key_to_binary}Finding the \emph{binary superinterval sequence} of $\mathbf k$ (left), \ie the longest sequence $\mathbf{K_2}$ (right) such that $\mathbf k \subseteq \mathbf {k_2}$. The procedure is equivalent to generating $\mathbf{K_2}$ from $\mathbf K$ using the interval algorithm, so $\mathbf{K_2}$ is \iid uniform by construction. There is a single possible binary superinterval sequence. In this example $\mathbf k = [0.28,0.39)$ and $\mathbf{K_2} = [0,1]$.}

\end{figure}

\cref{fig:plaintext_to_binary,fig:key_to_binary} demonstrate converting plaintext $\mathbf P$ and key $\mathbf K$ to their binary representations. $\mathbf{P_2} = [1,0,0]$ is a binary \emph{subinterval} sequence of $\mathbf p = [0.49,0.69)$ because it needs to uniquely identify the prefix $\mathbf P$.\footnote{Again, the longest sequence it identifies might be longer than $\mathbf P$, but it is guaranteed to contain $\mathbf P$ as a prefix.} Conversely, $\mathbf{K_2} = [0,1]$ is a binary \emph{superinterval} sequence of $\mathbf k = [0.28,0.39)$ because it needs to be uniquely identified using the key $\mathbf K$.

Ciphertext $\mathbf {C_2}$ is constructed by the \emph{exclusive or} operation on streams of $\mathbf{P_2}$ and the cycled key $\mathbf{K_2^+} = [0,1,0,1,0,1,\dots]$. Thus, it is the simplest and least secure implementation of a stream cipher. The ciphertext becomes $\mathbf {C_2} = [1,1,0]$.

Ciphertext $\mathbf {C_2}$ is converted to stegotext $\mathbf S$ using the system from \cref{sec:arbitrary_source_stego} as shown in
