...
$\mathbf{C_2}$ is used as an input to the simple stegosystem, producing pseudo-random English \emph{stegotext} $\mathbf S$. Provided that the language model is correct, the stegotext is statistically indistinguishable from any random English text.
...
However, the recovery of plaintext requires knowledge of the key. If a \emph{false key} $\mathbf {\overline K}$ is supplied in text form, it will still be correctly converted to its binary superinterval sequence $\mathbf {\overline K_2}$. It will then allow to decipher ciphertext $\mathbf{C'_2}$ into an \emph{incorrect} binary sequence $\mathbf {\overline {P'}_2} = \mathbf{C'_2} \oplus \mathbf {\overline K^+_2}$ without failure. $\mathbf {\overline {P'}_2}$ will then be mapped to some incorrect plaintext $\mathbf {\overline {P'}}$, generated according to the stochastic process $\mathcal P$. So it will be a fragment of text randomly drawn from all English sentences.
...
Assume that the language contains three tokens $\{ \alpha, \beta, \gamma \}$ and both plaintext and stegotext are generated according to one stochastic process $\mathcal E$. The plaintext we wish to securely and innocuously transmit is $\mathbf P = [\beta,\gamma]$. For this purpose we use the key $\mathbf K = [\beta,\alpha]$.

\cref{fig:plaintext_to_binary,fig:key_to_binary} demonstrate converting the plaintext and key intervals $\mathbf p = [0.49, 0.69)$ and $\mathbf k = [0.28, 0.39)$ to their binary representations $\mathbf{P_2} = [1,0,0]$ and $\mathbf{K_2} = [0,1]$. Note that $\mathbf{K_2}$ is shorter than $\mathbf{P_2}$, so perfect secrecy is not possible.
...
\begin{figure}[h]
	...
	\caption{\label{fig:plaintext_to_binary}Finding a binary subinterval sequence $\mathbf{P_2} = [1,0,0]$ of $\mathbf p = [0.49,0.69)$.}
...
\begin{figure}[h]
	...
	\caption{\label{fig:key_to_binary}Finding the binary superinterval sequence $\mathbf{K_2} = [0,1]$ of $\mathbf k = [0.28,0.39)$.}

\end{figure}

Ciphertext $\mathbf {C_2}$ is constructed by the \emph{exclusive or} operation on streams of $\mathbf{P_2}$ and the cycled key $\mathbf{K^+_2} = [0,1,0,1,0,1,\dots]$, \ie $\mathbf {C_2} = \mathbf{P_2} \oplus \mathbf{K^+_2} = [1,1,0]$. The ciphertext is used to generate stegotext $\mathbf S = [\gamma,\alpha,\beta]$ as shown in \cref{fig:ciphertext_to_stegotext}.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[scale=0.05]
		...
		\node[below] at (-30,-5) {\footnotesize $[1,1,0,\mathit{0}]$};
	...
	\caption{\label{fig:ciphertext_to_stegotext}Generating stegotext $\mathbf S = [\gamma,\alpha,\beta]$ from ciphertext $\mathbf{C_2} = [1,1,0]$. Ciphertext needs to be extended to $\mathbf I = [1,1,0,\mathit{0}]$ so that $\mathbf s \subseteq \mathbf{c_2}$. Note that the stegotext is not unique -- if we extended the ciphertext to $\mathbf I = [1,1,0,\mathit{1,1}]$, the stegotext would be $\mathbf S = [\gamma,\alpha,\gamma]$.}
