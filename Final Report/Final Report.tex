...
\subsection{Perplexities of plaintext, key and stegotext}

See \cite{brown1992entropy} for a broader discussion of perplexity in computational linguistics.

Perplexity per word is the expected negative log probability of a token given the previous tokens. In the context of the interval algorithm, it directly corresponds to the expected size of successive subintervals. It can be also interpreted as the number of bits of information on average needed to encode one symbol.

We can talk about perplexity of a probability distribution and of a probability model. The former is a feature of sentences generated

A good language model minimises the perplexity of test data -- it aims to predict successive tokens as well as possible. Thus, when choosing the parameters for plaintext model, we should aim to minimise the perplexity.
...
\subsection{Different models for plaintext and stegotext}
...
\subsubsection{Rate of the system}

Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.

\subsubsection{Entropy of the language model and its effect on stegotext length}
...
\label{sec:n5_beta01_gamma001_f39}
