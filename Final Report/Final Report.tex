...
Fig.~\ref{fig:backoff_tree} shows how to a construct a \emph{back-off tree} where each token is represented by a single leaf. Each leaf corresponds to a contiguous, non-zero interval. The set of all leaf intervals partitions $[0,1)$. As a result, the tree can be interpreted as a \pmf over all tokens. It also satisfies all requirements from the previous section.
