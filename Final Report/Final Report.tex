...
\subsubsection{Counts induced from extended 1-grams}
...
Consider processing the extended 1-gram from Tab.~\ref{tab:extended_1gram} as an example. 35 base $N$-grams listed in Tab.~\ref{tab:extended_1gram_induced_base_ngrams} can be found in it. Each should be inserted in its respective base $N$-gram counts table using the count of the extended 1-gram, in this case 58. Note that the hyphen token will be inserted twice. This is correct -- it should be assigned a total count of $2 \times 58 = 116$ because it occurs twice in the extended 1-gram.
