...
Fig.~\ref{fig:extended_and_base_ngrams} should make it clear why this is the case. Base $N$-grams \ngram{cross}, \ngram{-}, \ngram{section}, \ngram{cross -}, \ngram{- section} and \ngram{cross - section} occur as many times as the extended 1-gram \ngram{cross-section}.

As an example, consider processing the extended 1-gram from Tab.~\ref{tab:extended_1gram}. 35 base $N$-grams listed in Tab.~\ref{tab:extended_1gram_induced_base_ngrams} can be induced from it. Each should be inserted in its respective counts table using the count of the extended 1-gram, in this case 58. Note that the \ngram{-} token will be inserted twice. It indeed should be assigned a total count of $2 \times 58 = 116$, as it occurs twice in the extended 1-gram.
...
\begin{table}[h]
	...
	\end{tabular}
	\caption{\label{tab:extended_1gram_induced_base_ngrams}Base $N$-grams induced from the extended 1-gram from Tab.~\ref{tab:extended_1gram}.}
\end{table}
