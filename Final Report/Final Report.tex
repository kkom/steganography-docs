...
\begin{equation}
\label{eq:backoff_pseudocount_level1}
c(b_1) = \ceil*{ \beta \bracket*{ f(w_{l-N+1}^{l-1}) - \sum_{w_l \in \mathcal W_1} f(w_{l-N+1}^l) } + \gamma ~ f(w_{l-N+1}^{l-1}) }
\end{equation}

Parameter $\beta$ controls what proportion of the \emph{leftover probability mass} is used to account for the event of a back-off. $\gamma$ controls how much of the \emph{total probability mass} is added to account for back-off. To illustrate, the total probability mass in the example from Section~\ref{eq:lm_requirements} is $1146$ and the leftover probability mass is $1146-542=604$.

Dealing with excluded tokens in the deeper levels of the back-off tree is not difficult. It suffices to subtract their counts from the total probability mass and repeat the calculation from Eq.~\ref{eq:backoff_pseudocount_level1} in an analogous way:

\begin{align}
m &= f(w_{l-N+d}^{l-1}) - \sum_{w_l \in \mathcal W_1 \cup \dots \cup W_{d-1}} f(w_{l-N+d}^l) \\
\label{eq:backoff_pseudocount_deep_levels} c(b_d) &= \ceil*{ \beta \bracket*{ m - \sum_{w_l \in \mathcal W_d} f(w_{l-N+d}^l) } + \gamma ~ m }
\end{align}
