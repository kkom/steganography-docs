...
If we explode tokens in Tab.~\ref{tab:its_ngrams}, all $N$-grams will correspond to the same sequence of base tokens: \emph{it\ '\ s}. More generally, any phonological word that can be ambiguously split into multiple tokens because of internal punctuation can be uniquely identified by the sequence of its base tokens.
...
\emph{Extended $N$-gram} will signify a sequence of $N$ unnormalised extended tokens (as given in the Google Books counts database) and \emph{base $N$-gram} a sequence of $N$ normalised base tokens (what we wish to know the counts of).

\newcommand{\letterMark}[1] {
	\filldraw (#1-1,1) circle (3pt); \draw (#1-1,1)--(#1,1); \filldraw (#1,1) circle (3pt);
}
...
\end{tikzpicture}
\caption{\label{fig:extended_base_counts}Extended and base $N$-grams for $N \le 3$ in an example text.}
\end{figure}

\subsubsection{Induced counts from extended 1-grams}

Inducing the counts of base $N$-grams from extended 1-grams is easy. It suffices to normalise and explode the extended token to get the sequence of base tokens.
...
\begin{table}[h]
	...
	formaldehyde & & & & \\
	\end{tabular}
	\caption{\label{tab:extended_1gram_induced_base_ngrams}Base $N$-grams induced from the extended 1-gram.}
\end{table}

Each $N$-gram from Tab.~\ref{tab:extended_1gram_induced_base_ngrams} has to be inserted into the appropriate table of base $N$-gram counts using the count of the extended 1-gram, in this case 58. Note that the base token \emph{-} (hyphen) will be inserted twice. This is correct -- it should be assigned a count of $2 \times 58 = 116$ because it occurs twice in the extended 1-gram. Duplicate $N$-grams will be dealt with later.

\subsubsection{Induced counts from extended $N$-grams ($N > 1$)}
