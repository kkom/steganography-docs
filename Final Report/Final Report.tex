...
The system described in \cref{sec:arbitrary_source_stego} can be used as a part of a bigger system, which \emph{securely} and \emph{innocuously} transmits arbitrary English text $\mathbf P$ using different English text $\mathbf K$ as a \emph{key}. I will describe a modular stegosystem in which both the \emph{plaintext} and the \emph{key} are first converted to binary sequences $\mathbf{P_2}$ and $\mathbf{K_2}$. The conversion is achieved using the interval algorithm with the English language model, so the resulting binary sequences can be made \iid uniform.

The binary representation of plaintext $\mathbf{P_2}$ is then encrypted to binary \emph{ciphertext} $\mathbf{C_2}$ using a stream cipher and $\mathbf{K_2}$. The particular cipher used in this project maintains $\mathbf{C_2}$ as \iid uniform. $\mathbf{C_2}$ is then used as an input to the system from \cref{sec:arbitrary_source_stego}, producing pseudo-random English \emph{stegotext} $\mathbf S$. Provided that the language model is correct, the stegotext is statistically indistinguishable from any random English text.
