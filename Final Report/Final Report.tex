...
\begin{table}[h]
	...
	formaldehyde & & & & \\
	\end{tabular}
	\caption{\label{tab:extended_1gram_induced_base_ngrams}Induced $N$-grams from the extended 1-gram in Tab.~\ref{tab:extended_1gram}.}
\end{table}
...
Counts consistency is assumed in some parts of the stegosystem, for example by the $\beta - \gamma$ algorithm in Section \ref{sec:beta_gamma_backoff}. Since inconsistencies are very rare (in the order of 500 for the whole database of \noteSelf{how many?} $N$-grams), they can be corrected without visibly distorting information in the counts.

In the case of Google Books database, 5-grams are asserted to be counts consistent, since there are no 6-grams to check with. In general, $N$-grams are made left counts consistent in the following way:
