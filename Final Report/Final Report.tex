...
\begin{figure}[h]
	...
	\caption{\label{fig:stegosystem_decoding}Generating $\mathbf{P'}$ (right) from $\mathbf S$ (left). We use the statistics of $\mathcal S$ to find the interval $\mathbf s = [0.42,0.56)$. We then use the interval algorithm to generate $\mathbf{P'}$. Since we don't know how long the original sequence $\mathbf P$ was, we keep generating terms according to the statistics of $\mathcal P$ for as long as possible. In this case we decode $\mathbf{P'} = [b,c,a]$. $\mathbf{P'}$ has the prefix $\mathbf P = [b,c]$, but also contains an extra symbol $a$.}
...
Ciphertext $\mathbf {C_2}$ is converted to stegotext $\mathbf S$ as shown in \cref{fig:ciphertext_to_stegotext}.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[scale=0.05]
		% C_2 interval range
		\fill [opacity=0.1] (-30,0.750*100) rectangle (120,0.875*100);

		% I interval range
		\fill [opacity=0.1] (-30,0.7500*100) rectangle (120,0.8125*100);
		...
		\intervalBothLabels{-90}{0}{100}{0.75}{1.00}{solid}{left}
		\intervalBottomLabel{-90}{0}{100}{0.50}{0.75}{densely dotted}{left}
		\node[below] at (-90,-5) {\footnotesize $[1,1]$};

		\intervalBothLabels{-60}{0}{100}{0.875}{1.000}{densely dotted}{left}
		\intervalBottomLabel{-60}{0}{100}{0.750}{0.875}{solid}{left}
		\node[below] at (-60,-5) {\footnotesize $[1,1,0]$};

		\intervalBothLabels{-30}{0}{100}{0.8125}{0.8750}{densely dotted}{left}
		\intervalBottomLabel{-30}{0}{100}{0.7500}{0.8125}{solid}{left}
		...
		\intervalBottomLabel{60}{0}{100}{0.88}{0.94}{densely dotted}{right}
		\intervalBottomLabel{60}{0}{100}{0.69}{0.88}{solid}{right}
		\node[below] at (60,-5) {\footnotesize $[\gamma,\alpha]$};

		\intervalBothLabels{30}{0}{100}{0.82}{0.88}{densely dotted}{right}
		\intervalBottomLabel{30}{0}{100}{0.75}{0.82}{solid}{right}
		\intervalBottomLabel{30}{0}{100}{0.69}{0.75}{densely dotted}{right}
		\node[below] at (30,-5) {\footnotesize $[\gamma,\alpha,\beta]$};

	\end{tikzpicture}

	\caption{\label{fig:ciphertext_to_stegotext}Generating stegotext $\mathbf S = [\gamma,\alpha,\beta]$ from ciphertext $\mathbf{C_2} = [1,1,0]$. Ciphertext needs to extended to $\mathbf I = [1,1,0,0]$ so that $\mathbf s \subseteq \mathbf{c_2}$.}
