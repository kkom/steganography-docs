...
\begin{tikzpicture}[scale=0.5]
	...
	\draw[dashed] [shorten >= 1.5 pt] (9,20.5) -- (35,20.5);
	\draw[solid] [i] (35,12.5) -- (35,20.5);
	\draw[dashed] (18,12.5) -- (35,12.5);
	\draw[solid] [i] (35,7.5) -- (35,12.5);
	\draw[dashed] (15,7.5) -- (35,7.5);
	\draw[solid] [i] (35,5.5) -- (35,7.5);
	\draw[dashed] (12,5.5) -- (35,5.5);
	\draw[solid] [i] (35,1.5) -- (35,5.5);
	\draw[dashed] (9,1.5) -- (35,1.5);
	\draw[solid] [i] (35,-1.5) -- (35,1.5);
	\draw[dashed] (9,-1.5) -- (35,-1.5);

	% Label the interval ends
	\node[right] at (35,20.5) {\footnotesize 1};
	\node[right] at (35,-1.5) {\footnotesize 0};

%	% Define sets of leaves in a certain level (with braces)
%	\draw [decorate,decoration={brace,amplitude=5pt,raise=10pt,mirror}] (9,-1.4) -- (9,1.4) node [midway,right,xshift=15pt] {\footnotesize $\mathcal W_1 = \left\{ w_l : f(w_{l-4}^l) \ge C \right\}$ };
%	\draw [decorate,decoration={brace,amplitude=5pt,raise=10pt,mirror}] (12,1.6) -- (12,5.4) node [midway,right,xshift=15pt] {\footnotesize $\mathcal W_2 = \left\{ w_l : f(w_{l-3}^l) \ge C \land f(w_{l-4}^l) < C \right\}$ };
%	\draw [decorate,decoration={brace,amplitude=5pt,raise=10pt,mirror}] (15,5.6) -- (15,7.4) node [midway,right,xshift=15pt] {\footnotesize $\mathcal W_3 = \left\{ w_l : f(w_{l-2}^l) \ge C \land f(w_{l-3}^l) < C \right\}$ };
%	\draw [decorate,decoration={brace,amplitude=5pt,raise=10pt,mirror}] (18,7.6) -- (18,12.4) node [midway,right,xshift=15pt] {\footnotesize $\mathcal W_4 = \left\{ w_l : f(w_{l-1}^l) \ge C \land f(w_{l-2}^l) < C \right\}$ };
%	\draw [decorate,decoration={brace,amplitude=5pt,raise=10pt,mirror}] (21,12.6) -- (21,20.4) node [midway,right,xshift=15pt] {\footnotesize $\mathcal W_5 = \left\{ w_l : f(w_l) \ge C \land f(w_{l-1}^l) < C \right\}$ };

	% Define sets of leaves in a certain level
	\node [right,xshift=10pt] at (21,0) {\scriptsize $\mathcal W_1 = \left\{ w_l : f(w_{l-4}^l) \ge C \right\}$ };
	\node [right,xshift=10pt] at (21,3.5) {\scriptsize $\mathcal W_2 = \left\{ w_l : f(w_{l-3}^l) \ge C \land f(w_{l-4}^l) < C \right\}$ };
	\node [right,xshift=10pt] at (21,6.5) {\scriptsize $\mathcal W_3 = \left\{ w_l : f(w_{l-2}^l) \ge C \land f(w_{l-3}^l) < C \right\}$ };
	\node [right,xshift=10pt] at (21,10) {\scriptsize $\mathcal W_4 = \left\{ w_l : f(w_{l-1}^l) \ge C \land f(w_{l-2}^l) < C \right\}$ };
	\node [right,xshift=10pt] at (21,16.5) {\scriptsize $\mathcal W_5 = \left\{ w_l : f(w_l) \ge C \land f(w_{l-1}^l) < C \right\}$ };

	% Label the levels
	\node[below] at (9,-2) {\footnotesize level 1};
	\node[below] at (12,-2) {\footnotesize level 2};
	\node[below] at (15,-2) {\footnotesize level 3};
	\node[below] at (18,-2) {\footnotesize level 4};
	\node[below] at (21,-2) {\footnotesize level 5};
...
\end{tikzpicture}
\caption{\label{fig:backoff_tree}Example back-off tree for context $w_{l-4}^{l-1}$. Leaf labels $w_l \in 1, \dots, V$ correspond to token indices. $V=22$ in this example.}
\end{figure}

Fig.~\ref{fig:backoff_tree} shows how to a construct a \emph{back-off tree} where each token is represented by only one leaf.
