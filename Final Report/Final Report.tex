...
In this section I will describe the language model created for the purpose of the stegosystem. For the needs of arithmetic coding, which is the backbone of the stegosystem, it is necessary to have a model that returns probability of a token given its predecessors. Moreover, the tokens have to be ordered so that each corresponds to a unique subinterval of $[0,1)$ of size equal to the conditional probability of the token.

I will first state all the requirements formally and analyse their implications. I will then propose a model where all tokes are arranged in an $N$ level \emph{back-off tree}. The tree allows efficient calculation of conditional probability interval for a token without having to explicitly compute intervals of all other tokens.
...
For example, if we want to transmit information which is optimally compressed to binary, $\mathcal P$ will be a sequence of \iid uniform bits. If we want the stegotext to be English text, we will use the English language model from \cref{sec:language_model} as $\mathcal S$.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[scale=0.05]
		...
		\intervalBothLabels{90}{0}{100}{0.87}{1.00}{densely dotted}{right}
		\intervalBottomLabel{90}{0}{100}{0.32}{0.87}{solid}{right}
		\intervalBottomLabel{90}{0}{100}{0.18}{0.32}{densely dotted}{right}
		...
		\intervalBottomLabel{60}{0}{100}{0.32}{0.42}{densely dotted}{right}
		\node[below] at (60,-5) {\footnotesize $[\gamma, \beta]$};

	\end{tikzpicture}

	\caption{\label{fig:stegosystem_encoding}Generating $\mathbf S$ from $\mathbf P$. Both $\mathcal P$ and $\mathcal S$ are arbitrary stochastic processes over the alphabets $\mathfrak P = \{a,b,c\}$ and $\mathfrak S = \{\alpha,\beta,\gamma,\delta\}$. $\mathcal R$ gives a sequence of \iid uniform bits. We have $\mathbf P = [b,c]$ and we wish to generate $\mathbf S$ from it. $\mathbf P$ corresponds to the light grey region $\mathbf p = [0.37,0.69)$, which only allows to generate $\mathbf S$ up to $[\gamma]$. $[\gamma]$ is too short to identify a sequence from $\mathcal P$ with the prefix $\mathbf P$. However, if we use $\mathcal R$ to extend $\mathbf P$ to $\mathbf I = [b,c,0,1]$, we obtain the refined interval $\mathbf i = [0.45, 0.53)$ shown in dark grey. This interval allows us to generate $\mathbf S = [\gamma,\beta]$ with the required property that $\mathbf s \subseteq \mathbf p$.}
