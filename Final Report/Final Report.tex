...
The problem is that we have no control over how small the interval $\mathbf s$ that satisfies $\mathbf s \subseteq \mathbf p$ gets. We don't know when to switch from the alphabet $\mathfrak P$ to $\mathfrak R$, so we keep using the statistics of the stochastic process $\mathcal P$ to decode $\mathbf{P'}$ from $\mathbf S$ for as long as possible. It may happen that even the largest of the intervals satisfying this condition will allow us to decode \emph{more} than just $\mathbf P$. This is why the sequence we decode is $\mathbf{P'}$, not just $\mathbf P$. See \cref{fig:stegosystem_decoding} for an example of such a situation.
...
\begin{figure}[h]
	...
	\caption{\label{fig:stegosystem_decoding}Generating $\mathbf{P'}$ (right) from $\mathbf S$ (left). We use the statistics of $\mathcal S$ to find the interval $\mathbf s = [0.42,0.59)$ that corresponds to $\mathbf S$. We then use the interval algorithm to generate $\mathbf{P'}$. Since we don't know how long the original sequence $\mathbf P$ was, we keep generating terms according to the statistics of $\mathcal P$ for as long as possible. In this case we decode $\mathbf{P'} = [b,c,b]$. $\mathbf P = [b,c]$ is a prefix of $\mathbf{P'}$, but the latter also contains an extra symbol $b$.}
