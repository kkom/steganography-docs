...
\begin{figure}[h]
	...
	\caption{\label{fig:ciphertext_to_stegotext}Generating stegotext $\mathbf S = [\gamma,\alpha,\beta]$ from ciphertext $\mathbf{C_2} = [1,1,0]$. Ciphertext needs to be extended to $\mathbf I = [1,1,0,\mathit{0,0}]$ so that $\mathbf s \subseteq \mathbf{c_2}$, \ie $[0.75,0.81) \subseteq [0.750,0.875)$. Note that the stegotext is not unique -- if we extended the ciphertext to $\mathbf I = [1,1,0,\mathit{1,0}]$, the stegotext would be $\mathbf S = [\gamma,\alpha,\gamma]$.}

\end{figure}

\cref{fig:stegotext_to_ciphertxt} shows how the binary ciphertext sequence with trailing bits $\mathbf{C'_2} = [1,1,0,0]$ can be recovered from the stegotext $\mathbf S$. Since the recipient of the message knows $\mathbf K$, $\mathbf {K_2} = [0,1]$ can be obtained exactly like in \cref{fig:key_to_binary}. Deciphered binary plaintext with trailing bits is then $\mathbf {P'_2} = \mathbf{C'_2} \oplus \mathbf {K^+_2} = [1,0,0,1]$. We can verify that $\mathbf {P_2} = [1,0,0]$ is a prefix of $\mathbf {P'_2}$.
...
\begin{figure}[h]
	...
	\caption{\label{fig:stegotext_to_ciphertxt}Recovering the ciphertext with trailing bits $\mathbf{C'_2} = [1,1,0,0]$ from the stegotext $\mathbf S = [\gamma,\alpha,\beta]$. Note that the ciphertext $\mathbf {C_2} = [1,1,0]$ is indeed a prefix of $\mathbf{C'_2}$.}

\end{figure}

$\mathbf {P'_2}$ can now be used to recover $\mathbf{P'} = [\beta,\gamma,\beta]$ as in \cref{fig:binary_to_plaintext}. $\mathbf P = [\beta,\gamma]$ is indeed a prefix of $\mathbf {P'}$, so the stegosystem works correctly.
