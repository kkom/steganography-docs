...
Even though Eq.~\ref{eq:exact_string_probability} gives a convenient formulation of language as a stochastic process, it is computationally intractable. If the conditional probabilities are stored in tables, the conditional probability table of the $l$th token has $V^l$ entries. Total size of all tables becomes prohibitively large for even small $L$.
...
where $d(\cdot)$ and $\alpha(\cdot)$ return the discount and back-off weights -- scaling factors used to ensure that Eq.~\ref{eq:conditional_token_probability_with_backoff} gives a valid \pmf. Intuitively, they reduce the mass assigned to observed $N$-grams to make space for estimates of order $N{-}1$ and lower.
...
The Google Books $N$-grams database is based on many books, some of which include data tables. As a result, they contain a large number of $N$-grams consisting of just numbers and punctuation marks. Randomly generated text would sometimes include long sequences of those. To avoid such a possibility, I decided to create a second version of the database where $N$-grams containing digits are discarded. The only trade-off is that dates that use digits to write days of the month will no longer be a part of the language.
...
\emph{Extended $N$-gram} will refer to a sequence of $N$ unnormalised extended tokens (as stored in the Google Books counts database) and \emph{base $N$-gram} -- a sequence of $N$ normalised base tokens (counts of which we wish to know). See \cref{fig:extended_and_base_ngrams} for examples.
...
\end{tikzpicture}
\caption{\label{fig:extended_and_base_ngrams}Extended and base $N$-grams for $N \le 3$ in an example text. Colour alternates between red and black for consecutive base tokens.}
\end{figure}

\subsubsection{Inducing counts from extended 1-grams}

Inducing counts of base $N$-grams from extended 1-grams is easy. It is enough to generate base token sequences of the extended 1-grams and find all base $N$-grams in each of them -- let us call them \emph{induced $N$-grams}.

\cref{fig:extended_and_base_ngrams} should make it clear why this is the case. Base $N$-grams \ngram{cross}, \ngram{-}, \ngram{section}, \ngram{cross -}, \ngram{- section} and \ngram{cross - section} occur as many times as the extended 1-gram \ngram{cross-section}.

As an example, consider processing the extended 1-gram from \cref{tab:extended_1gram}. 35 base $N$-grams listed in \cref{tab:extended_1gram_induced_base_ngrams} can be induced from it. Each should be inserted in its respective counts table using the count of the extended 1-gram, in this case 58. Note that the \ngram{-} token will be inserted twice. It indeed should be assigned a total count of $2 \times 58 = 116$, as it occurs twice in the extended 1-gram.
...
\begin{table}[h]
	...
	\hline
	\scriptsize d & \scriptsize d : & \scriptsize d : diazo & \scriptsize d : diazo - & \scriptsize d : diazo - oxide \\
	\scriptsize : & \scriptsize : diazo & \scriptsize : diazo - & \scriptsize : diazo - oxide & \scriptsize : diazo - oxide / \\
	\scriptsize diazo & \scriptsize diazo - & \scriptsize diazo - oxide & \scriptsize diazo - oxide / & \scriptsize diazo - oxide / phenol \\
	\scriptsize - & \scriptsize - oxide & \scriptsize - oxide / & \scriptsize - oxide / phenol & \scriptsize - oxide / phenol - \\
	\scriptsize oxide & \scriptsize oxide / & \scriptsize oxide / phenol & \scriptsize oxide / phenol - & \scriptsize oxide / phenol - formaldehyde \\
	\scriptsize / & \scriptsize / phenol & \scriptsize / phenol - & \scriptsize / phenol - formaldehyde & \\
	\scriptsize phenol & \scriptsize phenol - & \scriptsize phenol - formaldehyde & & \\
	\scriptsize - & \scriptsize - formaldehyde & \scriptsize  & & \\
	\scriptsize formaldehyde & \scriptsize & & & \\
	\end{tabular}
	\caption{\label{tab:extended_1gram_induced_base_ngrams}Base $N$-grams induced from the extended 1-gram from \cref{tab:extended_1gram}.}
\end{table}
...
Inducing counts from extended $N$-grams for $N>1$ is similar to the case when $N=1$. An equivalent sequence of base tokens has to be constructed as well, but \emph{not all} possible induced $N$-grams can be selected. If a base $N$-gram can be also induced from an extended $M$-gram, it \emph{cannot} be selected. If we did select it while processing the extended $N$-gram, it would be double counted -- we are guaranteed to come across its count again when processing the offending $M$-gram.
...
$N$-grams are \emph{counts consistent} if both Eq.~\ref{eq:left_counts_consistency} and Eq.~\ref{eq:right_counts_consistency} are satisfied.
...
Counts consistency is assumed in some parts of the stegosystem, for example by the $\beta - \gamma$ algorithm in Section \ref{sec:beta_gamma_backoff}. Since inconsistencies are very rare (in the order of \num{1000} for the whole database of about \num{1500000000} $N$-grams), they can be corrected without visibly distorting information in the counts.
...
Requirement \ref{lm_req:non_zero_probability} warrants the use of back-off. Consider the counts from \cref{tab:the_esteem_of_many_CONT} and \cref{tab:the_esteem_of_many}. In the 5-gram model, only 5 tokens are explicitly allowed to follow \ngram{the esteem of many} and their total count is 542. If we used the ML model from Eq.~\ref{eq:ml_conditional_token_probability} to calculate their probabilities, we would assign zero probability to all tokens other than those in column $w_5$ of \cref{tab:the_esteem_of_many_CONT}. Thus, we would account for only $\frac {542} {1146} \approx 0.47$ of the probability mass and also discard the vast majority of tokens. The former problem can be mitigated by upscaling the probabilities, but the latter has no solution other than back-off.
